...
<#
$lines_reg = @(Select-String -Path C:\temp\SAMPLES\2017\reg004001r.prt -Pattern "\d{3}-\d{3}")
$lines_main = @(Select-String -Path C:\temp\SAMPLES\2017\ord004001m.prt -Pattern "\d{3}-\d{2}-\d{4}")
...
$orders |  Select DATED, LAST_NAME, FIRST_NAME, MIDDLE_INITIAL, SSN, UIC, ORDER_NUMBER, PERIOD_FROM, PERIOD_TO, FORMAT | Format-Table -AutoSize -Wrap
#>