function Undo-PreviousSessions()
{
    ...
    if($($csvs_in_path.Count) -gt 0)
    {
        ...
        $files = Get-ChildItem -Path "C:\temp\OUTPUT\UICS" -Exclude "__PERMISSIONS" -Recurse -Include "*.txt" | Select Directory, Name
        $dir_file = @{}
